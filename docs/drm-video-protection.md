...
> **Status: archived proposal — not currently implemented or active in production.**
> The active playback path is the legacy signed MP4 flow. Reconsider this design
> only after S-Class selects a managed DRM provider, obtains commercial approval
> and credentials, packages encrypted assets, operates license infrastructure,
> and completes FairPlay certificate/setup work. The additive DRM metadata
> columns are dormant and backward-compatible; this document describes the
> former proof of concept and future requirements, not a deployed capability.
...
| Student content fetch | `src/features/lessons/hooks/useSecureContent.ts` and `packages/api/src/secureContent.ts` | The active path calls `get-signed-urls` and receives a short-lived MP4 URL. |
...
| Free previews | `lessons.is_free_preview`, `packages/api/src/accessControl.ts`, and the access matrix in `get-signed-urls` | Preview lessons can be played without a subscription through the active legacy signed-URL flow. |
...
| Sequential locking | `src/features/subjects/components/curriculum.tsx` and `src/pages/LessonPage.tsx` | Unlock decisions use `lesson_progress.is_watched`; the active legacy player preserves the existing progress callbacks and lesson navigation behavior. |
...
There is no active HLS/DASH packaging, encryption key service, Widevine/FairPlay license integration, or DRM-capable player. The existing `useContentProtection` and `useScreenRecordingDetection` hooks are heuristics only and remain separate from DRM.
...
Right-click blocking, shortcut interception, transparent overlays, focus-loss pausing, extension detection, and CSS/JavaScript screenshot tricks are not DRM. Existing deterrents are kept separate from authorization, and native controls remain available.
...
The former proof of concept evaluated Shaka Player for adaptive DASH/HLS and EME-based DRM integration; provider-specific FairPlay transforms still require validation against the selected vendor.

## Archived proof-of-concept integration boundary (not active)

The following files belonged to the archived vendor-neutral proof of concept and
are preserved in the `archive/drm-foundation-poc` Git branch for reference:
...
The example is an interface shape only; it is not a provider API or credential.
No broker URL, broker token, provider credential, encrypted asset, license
service, or FairPlay setup is active in the current codebase.
...
The existing watched threshold and sequential locking remain unchanged: obtaining
a signed legacy URL never writes `lesson_progress`; only the existing completion
action does.
...
## Future production prerequisites (not configured)

Before any future DRM activation, configure these in Supabase Edge Function
secrets/configuration for each deployment environment:
...
No DRM secret belongs in the landing or portal `.env` files. DRM is not active;
the dormant columns are not read by runtime playback. Any future activation
requires a managed provider, encrypted assets, license infrastructure,
credentials, FairPlay certificates/setup, and separate commercial approval.
